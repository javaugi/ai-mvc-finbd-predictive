...
import static com.jvidia.aimlbda.config.RestTemplateConfig.OLLAMA_API;
import static com.jvidia.aimlbda.config.RestTemplateConfig.REST_TEMPLATE;
...
public class OllamaApiService {
    ...
    private final int RESP_TIMEOUT = 120;
    ...
    public Mono<String> queryOllamaByWebClient(OllamaRequest ollamaRequest) {
        Map<String, Object> requestBody = Map.of(
                "model", ollamaProperties.getModel(), // Change to your model
                "messages", List.of(
                        Map.of("role", "user", "content", ollamaRequest.prompt())
                ),
                "stream", false
        );
        log.debug("queryOllamaByWebClient requestBody {}", requestBody);
        return WebClient
            .builder()
            .clientConnector(new ReactorClientHttpConnector(HttpClient.create()
                        .responseTimeout(Duration.ofSeconds(RESP_TIMEOUT))))
                .build()
            .post()
            .uri(OLLAMA_API)
    ...
    public Mono<String> queryOllamaByTemplate(OllamaRequest ollamaRequest) {
        ...
        String requestBody = """
            ...
            """.formatted(ollamaProperties.getModel(), ollamaRequest.prompt());
        log.debug("queryOllamaByTemplate ollamaRequest {} \n requestBody {}", ollamaRequest, requestBody);
