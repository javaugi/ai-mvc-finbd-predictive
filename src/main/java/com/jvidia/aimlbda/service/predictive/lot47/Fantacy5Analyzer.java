...
@Slf4j
public class Fantacy5Analyzer {

    private static final int N = 5;

    public static void main(String[] args) {
        Fantacy5Analyzer analyzer = new Fantacy5Analyzer();
        analyzer.analyzeLottoData();
    }

    public void analyzeLottoData() {
        List<LottoDraw> draws = CSVDataParser.getFantacy5Data();

        log.info("=== Michigan Fantacy 5 Analysis ===");
        log.info("Total regular drawings analyzed: " + draws.size());
        log.info("Date range: 01/01/2020 to 10/30/2025\n");
    ...
    private void analyzeIndividualNumbers(List<LottoDraw> draws) {
        ...
        for (int i = 0; i < Math.min(15, sortedNumbers.size()); i++) {
    ...
    private void analyzeNumberPairs(List<LottoDraw> draws) {
        ...
        System.out.println("=== TOP 10 NUMBER PAIRS ===");
    ...
    private void analyzeNumberTriplets(List<LottoDraw> draws) {
        ...
        System.out.println("=== TOP 8 NUMBER TRIPLETS ===");
        List<Map.Entry<String, Integer>> sortedTriplets = tripletFrequency.entrySet()
                .stream()
                .sorted((a, b) -> b.getValue().compareTo(a.getValue()))
                .collect(Collectors.toList());

        for (int i = 0; i < Math.min(8, sortedTriplets.size()); i++) {
    ...
    private void generateLikelyCombinations(List<LottoDraw> draws) {
        ...
        List<Integer> topNumbers = getTopFrequentNumbers(draws, 12);
        System.out.println("=== Top 12 Numbers === \n " + topNumbers);
    ...
    protected int[] generateTripletBasedCombinationV1(List<LottoDraw> draws, List<Integer> topNumbers) {
        ...
        for (int i = 0; i < sortedTriplets.size(); i++) {
            for (int j = i + 1; j < sortedTriplets.size(); j++) {
                ...
                if (combined.size() > 5) {
                    int[] result = selectBestFive(combined, topNumbers);
                    Arrays.sort(result);
                    return result;
                }
            }
        ...
        return completeTripletToFive(sortedTriplets.get(0), topNumbers);
    }
    ...
    private Map<String, Integer> analyzeQuadrupletFrequencies(List<LottoDraw> draws) {
        Map<String, Integer> quadrupletFrequency = new HashMap<>();
                ...
                for (int j = i + 1; j < numbers.length; j++) {
                    for (int k = j + 1; k < numbers.length; k++) {
                        for (int l = k + 1; l < numbers.length; l++) {
                            ...
                            quadrupletFrequency.put(quadrupletKey, quadrupletFrequency.getOrDefault(quadrupletKey, 0) + 1);
                        }
                    }
                }
            }
    ...
    private int[] completeTripletToFive(int[] triplet, List<Integer> topNumbers) {
        Set<Integer> combination = new HashSet<>();
        for (int num : triplet) {
            combination.add(num);
        }
    ...
    private int[] selectBestFive(Set<Integer> numbers, List<Integer> topNumbers) {
    ...
    private void generateLikelyCombinationsMy(List<LottoDraw> draws) {
        ...
        // Get most frequent numbers
        List<Integer> topNumbers = getTopFrequentNumbers(draws, 12);
        System.out.println("=== Top 12 Numbers RESULTS === \n " + topNumbers);
        ...
        // No QUADRUPLETS available from Fantacy 5 (1 - 39)
        System.out.println();
        ...
        int[] combo4 = generatePairBasedCombination(pairs, 0, topNumbers.get(0));
        int[] combo5 = generatePairBasedCombination(pairs, 1, topNumbers.get(1));
    ...
    private int[] generatePairBasedCombination(List<String> pairs, int n, Integer top) {
        // This is a simplified version - you can enhance this with actual pair analysis
        Random random = new Random();
