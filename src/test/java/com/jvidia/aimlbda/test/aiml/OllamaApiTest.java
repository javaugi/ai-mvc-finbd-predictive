...
Why This Happens
    RANDOM_PORT: Starts a real server, WebTestClient makes actual HTTP calls
    MOCK environment: Uses mock server, allows mutateWith() operations
    The mockJwt() mutator only works with mock server environments

check to see if Ollama is running 
...
@SpringBootTest
@ActiveProfiles("test")
@Disabled("Temporarily disabled for CICD - all tests pass without issues but time-consuming with timedout")
public class OllamaApiTest {
    long customTimeout = 600000L; //1 minute

    // Create a request body as a Map, which will be serialized to JSON
    String content = """
                {
                    "prompt": "Explain quantum computing in one brief sentence"
                }
            """;
    ...
    public void testByWebClientWithOAuth() throws Exception {
        MvcResult asyncResult = mockMvc.perform(post("/api/ollama")
                .contentType(MediaType.APPLICATION_JSON)
                .content(content)
                .with(oauth2Login()) // <— correct for MVC
        ).andDo(print())
        ...
        if (asyncResult.getResponse() != null) {
            ...
            System.out.println("testByWebClientWithOAuth /api/ollama \n Response: " + responseBody);
        }
    ...
    public void testByRestTemplateWithOAuth() throws Exception {
        MvcResult asyncResult = mockMvc.perform(post("/api/ollama/query")
                .contentType(MediaType.APPLICATION_JSON)
                .content(content)
                .with(oidcLogin()) // <— correct for MVC
        ).andDo(print())
